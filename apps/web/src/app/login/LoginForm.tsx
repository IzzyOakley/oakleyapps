  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      if (!email.endsWith('@oakleyhomebuilders.com')) {
        await auth.signOut()
        setError('Only @oakleyhomebuilders.com accounts are permitted.')
        return
      }
  ...
  return (
    <div>
      <button
        ...
        disabled={loading}
        className="w-full h-11 bg-white text-gray-900 font-medium rounded-lg flex items-center justify-center gap-3 transition-all duration-150 hover:scale-[1.01] hover:shadow-md active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
      >
        {loading ? (
