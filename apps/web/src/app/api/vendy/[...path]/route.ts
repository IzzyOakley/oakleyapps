...
async function proxyToTakeoffAgent(req: NextRequest, params: { path: string[] }): Promise<NextResponse> {
  ...
  const upstreamUrl = `${TAKEOFF_SERVICE_URL}/${path}${search}`

  // Pass verified user identity as trusted headers — the Next.js layer is the auth boundary.
  // The takeoff-agent trusts these headers when the internal secret is present.
  ...
  try {
    ...
    const hint = process.env.NODE_ENV === 'development'
      ? 'Takeoff agent is not running. Open a new terminal and run: bash start-dev.sh (from the services/takeoff-agent directory)'
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  return proxyToTakeoffAgent(req, await params)
}
