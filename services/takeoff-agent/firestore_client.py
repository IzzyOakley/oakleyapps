...
def get_project(project_id: str) -> dict | None:
    db = get_db()
    doc = db.collection("apps").document("shared").collection("projects").document(project_id).get()
    if not doc.exists:
        return None
...
def create_project(job_name: str, address: str) -> str:
    ...
    # Ensure uniqueness
    ref = db.collection("apps").document("shared").collection("projects").document(project_id)
    ref.set({
        "job_name": job_name,
        "address": address,
        "status": "open",
...
def update_project_blueprint(project_id: str, gcs_path: str) -> None:
    db = get_db()
    db.collection("apps").document("shared").collection("projects").document(project_id).update({
...
def list_jobs_by_projects(project_ids: list[str]) -> dict[str, dict]:
    ...
    for start in range(0, len(project_refs), chunk_size):
        ...
        docs = (
            db.collection("apps").document("vendy").collection("jobs")
...
def get_job(job_id: str) -> dict | None:
    db = get_db()
    doc = db.collection("apps").document("vendy").collection("jobs").document(job_id).get()
    if not doc.exists:
        return None
...
def update_job_status(job_id: str, status: str, error: str | None = None) -> None:
    ...
    if error:
        update["error"] = error
    db.collection("apps").document("vendy").collection("jobs").document(job_id).update(update)
...
def update_item_in_job(job_id: str, item_id: str, pm_override: float | None, notes: str | None, status: str) -> bool:
...
def write_approved_takeoff(project_id: str, job_id: str, project: dict, takeoff_data: dict, approved_by: str) -> None:
    db = get_db()
    db.collection("apps").document("shared").collection("takeoffs").document(project_id).set({
