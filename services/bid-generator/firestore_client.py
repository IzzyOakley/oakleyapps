...
def list_biddable_cost_codes() -> list[dict]:
    ...
    return [
        ...
        if c.get("flags", {}).get("biddable") and c.get("vendors")
    ]


def get_vendor(vendor_id: str) -> dict | None:
    db = get_db()
    doc = db.collection("apps").document("vendy").collection("vendors").document(vendor_id).get()
    if not doc.exists:
        return None
...
def create_bid(project_id, project_name, vendor_id, vendor_name, cost_code, cost_code_name) -> str:
    db = get_db()
...
def get_bid(bid_id) -> dict | None:
    db = get_db()
    doc = db.collection("apps").document("vendy").collection("bids").document(bid_id).get()
    if not doc.exists:
        return None
...
def list_bids_for_project(project_id) -> list[dict]:
    db = get_db()
    docs = (
        db.collection("apps").document("vendy").collection("bids")
